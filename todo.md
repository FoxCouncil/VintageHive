...
- [ ] **Related, still open - shared TemplateOptions is mutated per request.** `new TemplateContext(model)` uses the static `TemplateOptions.Default` (that's where the custom filters/`UnsafeMemberAccessStrategy` live), and `ControllerManager` sets `Context.Options.FileProvider` on it per request. Under concurrent requests to *different* domains this races (one render can pick up another domain's FileProvider). Not the reported bug and not triggered in the single-client retro use case, but worth a proper fix eventually (per-request `TemplateOptions` instead of mutating the shared default).
