...
internal partial class CookEncoder
{
    ...
    private readonly float[] _fftTwiddles;   // [N/2] complex twiddle factors for FFT stages (bit-reversed order (reference encoder style))
    ...
    public CookEncoder(int sampleRate, int channels, int bitrate)
    {
        ...
        // Reference: fsin(((i - (-0.5)) * π) / (2 * mdct_size))
        for (int i = 0; i < N; i++)
        {
        ...
        // Reference: fcos((i * π) / mdct_size) * sqrt(2.0 / mdct_size)
    ...
    private byte[] EncodeChannel(float[] pcmSamples, int channelIndex)
    {
        ...
        // (cook_apply_lpc_encoder copies, then conditionally calls cook_lpc_filter)
        // ---------------------------------------------------------------
        ...
        // Use double accumulation to match x87 FPU behavior: the 32-bit x86 encoder keeps
    ...
    private void ApplyMdct(float[] input2N)
    {
        ...
        // Matches cook_fft_compute exactly: radix-4 first pass,
    ...
    {
        // Reference: sum starts at 0.0 (no epsilon), nrg is already the sum of coeff^2
        // Then: scaled = sum * 0.05
        float scaled = nrg * 0.05f;

        // Reference: default multiplier = sqrt(0.5); if (1.0 < scaled) multiplier = sqrt(2)
        float val;
        if (scaled > 1.0f)
        {
    ...
    // Matches cook_apply_filter_coeffs exactly.
    ...
    {
        // Use double intermediates to match x87 FPU behavior: the 32-bit x86 encoder keeps
    ...
    private static void QuantizeGains(float[] currPeaks, float[] prevPeaks, int[] outGainInfo)
    {
        ...
        // Reference encoder normalizes each peak by peaks[8] (= subband 6 peak)
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            int exp = ((bits >> 23) & 0xFF) - 127;
            exp = exp - (exp >> 31);
            prevLog[i] = exp; // reference encoder does NOT clamp prevLog
        }
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            // with ascending previous pattern (cook_quantize_gains)
        ...
        int samplesPerChannel, int direction)
    {
        ...
        // Reference: cook_apply_subband_gain(buf+spc, gs[sub], gs[sub+1]) where gs[8]=0
        ...
        // Reference encoder reuses the same array, so gain_schedule[0] IS prevSchedule[0] at this point
        offset = 0;
        for (int sub = 0; sub < 8; sub++)
        {
    ...
    private static void EncodeTemporalGain(CookBitWriter bw, int[] gainInfo)
    {
        ...
        // Write gain count as unary code (matching cook_encode_envelope)
